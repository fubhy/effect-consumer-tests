...
const ChannelOpCodes = /*#__PURE__*/require("../opCodes/channel.js");
const ChildExecutorDecisionOpCodes = /*#__PURE__*/require("../opCodes/channelChildExecutorDecision.js");
const ChannelStateOpCodes = /*#__PURE__*/require("../opCodes/channelState.js");
const UpstreamPullStrategyOpCodes = /*#__PURE__*/require("../opCodes/channelUpstreamPullStrategy.js");
const ContinuationOpCodes = /*#__PURE__*/require("../opCodes/continuation.js");
const ChannelState = /*#__PURE__*/require("./channelState.js");
const Continuation = /*#__PURE__*/require("./continuation.js");
const Subexecutor = /*#__PURE__*/require("./subexecutor.js");
const upstreamPullRequest = /*#__PURE__*/require("./upstreamPullRequest.js");
class ChannelExecutor {
            ...
            if (Effect.isEffect(this._currentChannel)) {
              ...
              switch (this._currentChannel._tag) {
                      ...
                      const drainer = () => Effect.flatMap(bridgeInput.awaitRead(), () => Effect.suspend(() => {
                        ...
                        switch (state._tag) {
                          ...
                          case ChannelStateOpCodes.OP_READ:
                            {
                              return (0, exports.readUpstream)(state, () => drainer(), cause => bridgeInput.error(cause));
                            }
                        }
                      }));
                ...
                case ChannelOpCodes.OP_CONCAT_ALL:
                  {
                    const executor = new ChannelExecutor(this._currentChannel.value(), this._providedEnv, effect => Effect.sync(() => {
                      ...
                      this._closeLastSubstream = (0, Function_js_1.pipe)(prevLastClose, Effect.zipRight(effect));
                    }));
                ...
                case ChannelOpCodes.OP_FROM_EFFECT:
                  {
                    const effect = this._providedEnv === undefined ? this._currentChannel.effect() : (0, Function_js_1.pipe)(this._currentChannel.effect(), Effect.provide(this._providedEnv));
                ...
                case ChannelOpCodes.OP_READ:
                  {
                    ...
                    result = ChannelState.Read(this._input, Function_js_1.identity, emitted => {
                      try {
  ...
  close(exit) {
    ...
    if (finalizer !== undefined) {
      runInProgressFinalizers = (0, Function_js_1.pipe)(finalizer, Effect.ensuring(Effect.sync(() => this.clearInProgressFinalizer())));
    }
    let closeSelf = undefined;
    const selfFinalizers = this.popAllFinalizers(exit);
    if (selfFinalizers !== undefined) {
      closeSelf = (0, Function_js_1.pipe)(selfFinalizers, Effect.ensuring(Effect.sync(() => this.clearInProgressFinalizer())));
    }
    ...
    return (0, Function_js_1.pipe)(Effect.exit(ifNotNull(closeSubexecutors)), Effect.zip(Effect.exit(ifNotNull(runInProgressFinalizers))), Effect.zip(Effect.exit(ifNotNull(closeSelf))), Effect.map(([[exit1, exit2], exit3]) => (0, Function_js_1.pipe)(exit1, Exit.zipRight(exit2), Exit.zipRight(exit3))), Effect.uninterruptible,
    // TODO: remove
  ...
  doneSucceed(value) {
    ...
    const effect = (0, Function_js_1.pipe)(finalizerEffect, Effect.ensuring(Effect.sync(() => this.clearInProgressFinalizer())), Effect.uninterruptible, Effect.flatMap(() => Effect.sync(() => this.doneSucceed(value))));
  ...
  doneHalt(cause) {
    ...
    this.storeInProgressFinalizer(finalizerEffect);
    const effect = (0, Function_js_1.pipe)(finalizerEffect, Effect.ensuring(Effect.sync(() => this.clearInProgressFinalizer())), Effect.uninterruptible, Effect.flatMap(() => Effect.sync(() => this.doneHalt(cause))));
    return ChannelState.FromEffect(effect);
  }
  processCancellation() {
    this._currentChannel = undefined;
  ...
  provide(effect) {
    ...
    return (0, Function_js_1.pipe)(effect, Effect.provide(this._providedEnv));
  }
  ...
  finishSubexecutorWithCloseEffect(subexecutorDone, ...closeFuncs) {
    this.addFinalizer(() => (0, Function_js_1.pipe)(closeFuncs, Effect.forEach(closeFunc => (0, Function_js_1.pipe)(Effect.sync(() => closeFunc(subexecutorDone)), Effect.flatMap(closeEffect => closeEffect !== undefined ? closeEffect : Effect.unit)), {
      discard: true
    })));
    const state = (0, Function_js_1.pipe)(subexecutorDone, Exit.match({
      onFailure: cause => this.doneHalt(cause),
      onSuccess: value => this.doneSucceed(value)
    }));
    this._activeSubexecutor = undefined;
  ...
  pullFromChild(childExecutor, parentSubexecutor, onEmitted, subexecutor) {
    return ChannelState.Read(childExecutor, Function_js_1.identity, emitted => {
  ...
  performPullFromUpstream(subexecutor) {
    return ChannelState.Read(subexecutor.upstreamExecutor, effect => {
      ...
      this._closeLastSubstream = undefined;
      return (0, Function_js_1.pipe)(this._executeCloseLastSubstream(closeLastSubstream), Effect.zipRight(effect));
    }, emitted => {
      if (this._closeLastSubstream !== undefined) {
        const closeLastSubstream = this._closeLastSubstream;
        this._closeLastSubstream = undefined;
        return (0, Function_js_1.pipe)(this._executeCloseLastSubstream(closeLastSubstream), Effect.map(() => {
      ...
      if (subexecutor.activeChildExecutors.some(subexecutor => subexecutor !== undefined)) {
        ...
        if (this._closeLastSubstream !== undefined) {
          const closeLastSubstream = this._closeLastSubstream;
          this._closeLastSubstream = undefined;
          return (0, Function_js_1.pipe)(this._executeCloseLastSubstream(closeLastSubstream), Effect.map(() => this.replaceSubexecutor(drain)));
        }
        this.replaceSubexecutor(drain);
        return undefined;
      }
      const closeLastSubstream = this._closeLastSubstream;
      const state = this.finishSubexecutorWithCloseEffect((0, Function_js_1.pipe)(exit, Exit.map(a => subexecutor.combineWithChildResult(subexecutor.lastDone, a))), () => closeLastSubstream, exit => subexecutor.upstreamExecutor.close(exit));
...
const runFinalizers = (finalizers, exit) => {
  return (0, Function_js_1.pipe)(Effect.forEach(finalizers, fin => Effect.exit(fin(exit))), Effect.map(exits => (0, Function_js_1.pipe)(Exit.all(exits), Option.getOrElse(() => Exit.unit))), Effect.flatMap(exit => Effect.suspend(() => exit)));
};
/**
 * @internal
 */
      ...
      case ChannelStateOpCodes.OP_EMIT:
        {
          ...
          if (readStack.length === 0) {
            ...
            return (0, Function_js_1.pipe)(emitEffect, Effect.matchCauseEffect({
              onFailure,
              onSuccess
            }));
          }
      ...
      case ChannelStateOpCodes.OP_DONE:
        {
          ...
          if (readStack.length === 0) {
            ...
            return (0, Function_js_1.pipe)(doneEffect, Effect.matchCauseEffect({
              onFailure,
              onSuccess
            }));
          }
      ...
      case ChannelStateOpCodes.OP_FROM_EFFECT:
        {
          ...
          return (0, Function_js_1.pipe)(current.onEffect(state.effect), Effect.catchAllCause(cause => Effect.suspend(() => {
...
/** @internal */
const run = self => (0, Function_js_1.pipe)((0, exports.runScoped)(self), Effect.scoped);
exports.run = run;
/** @internal */
const runScoped = self => {
  const run = (channelDeferred, scopeDeferred, scope) => Effect.acquireUseRelease(Effect.sync(() => new ChannelExecutor(self, void 0, Function_js_1.identity)), exec => Effect.suspend(() => (0, Function_js_1.pipe)(runScopedInterpret(exec.run(), exec), Effect.intoDeferred(channelDeferred), Effect.zipRight(Deferred.await(channelDeferred)), Effect.zipLeft(Deferred.await(scopeDeferred)))), (exec, exit) => {
  ...
  return Effect.uninterruptibleMask(restore => Effect.flatMap(Effect.scope, parent => (0, Function_js_1.pipe)(Effect.all([Scope.fork(parent, ExecutionStrategy.sequential), Deferred.make(), Deferred.make()]), Effect.flatMap(([child, channelDeferred, scopeDeferred]) => (0, Function_js_1.pipe)(Effect.forkScoped(restore(run(channelDeferred, scopeDeferred, child))), Effect.flatMap(fiber => (0, Function_js_1.pipe)(Effect.addFinalizer(() => Deferred.succeed(scopeDeferred, void 0)), Effect.zipRight(restore(Deferred.await(channelDeferred))), Effect.zipLeft(Fiber.inheritAll(fiber)))))))));
};
...
const runScopedInterpret = (channelState, exec) => {
  ...
  switch (op._tag) {
    case ChannelStateOpCodes.OP_FROM_EFFECT:
      {
        return (0, Function_js_1.pipe)(op.effect, Effect.flatMap(() => runScopedInterpret(exec.run(), exec)));
      }
    case ChannelStateOpCodes.OP_EMIT:
      {
    ...
    case ChannelStateOpCodes.OP_READ:
      {
        return (0, exports.readUpstream)(op, () => runScopedInterpret(exec.run(), exec), Effect.failCause);
      }
  }
};
